...
import { Tabs, Segmented, Avatar } from "@/components/ui/primitives";
...
export default function StockResearch({ params }: { params: Promise<{ symbol: string }> }) {
  ...
  const { stock, history: bars } = useApi<{ stock: Stock | null; history: Candle[] }>(
    `/api/stock/${symbol}`, { stock: null, history: [] }, (j) => ({ stock: j.stock, history: j.history ?? [] })
  );
    ...
    <div>
      ...
      <Card variant="elevated" className="mb-6 overflow-hidden">
        <div className="relative p-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div className="mt-3 flex items-end gap-3">
                  <span className="text-[32px] font-bold text-frost tnum leading-none">{inr(stock.price)}</span>
                  <DeltaPill value={stock.change} pct={stock.changePct} className="mb-1" />
                </div>
                <div className="mt-2 flex items-center gap-3 text-[11px] text-muted">
                  <span>{stock.sector}</span><span>·</span>
                  <span>Mkt Cap ₹{compactCr(stock.mcapCr * 1e7)}</span><span>·</span>
          ...
          <div className="relative mt-6 grid grid-cols-2 gap-4 rounded-[var(--radius-md)] bg-void/50 p-4 hairline sm:grid-cols-4 lg:grid-cols-6">
            ...
            <RatingTile label="Health Score"><div className="w-full"><div className="text-[16px] font-bold text-frost tnum">{stock.health}<span className="text-[11px] text-muted">/100</span></div><HealthBar value={stock.health} /></div></RatingTile>
            <RatingTile label="P/E · P/B"><span className="text-[16px] font-bold text-frost tnum">{stock.pe} · {stock.pb}</span></RatingTile>
            <RatingTile label="ROE"><span className="text-[16px] font-bold text-frost tnum">{stock.roe}%</span></RatingTile>
            ...
            <RatingTile label="Div Yield"><span className="text-[16px] font-bold text-frost tnum">{stock.divYield}%</span></RatingTile>
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <div className="space-y-6">
          <Card variant="ai">
            <CardHeader icon={<Sparkles size={16} className="text-ai" />} title="AI Score" subtitle="Momentum-based (0-10)" action={<AiBadge>Live</AiBadge>} />
            <CardBody className="space-y-4">
              <p className="text-[13px] leading-relaxed text-mist">
                ...
                real price momentum{stock.rsi != null ? <> and RSI ({stock.rsi.toFixed(0)})</> : ""}. Momentum is
  ...
  return (
    <div className="grid gap-x-8 gap-y-0 sm:grid-cols-2">
      <div>
        ...
        <KV k="Market Cap" v={`₹${compactCr(stock.mcapCr * 1e7)}`} />
      ...
      <div>
        ...
        <KV k="Debt / Equity" v={`${stock.debtEquity}×`} tone={stock.debtEquity > 1 ? "text-warn" : "text-up"} />
...
function Technicals({ stock, bars }: { stock: Stock; bars: Candle[] }) {
  ...
  const rows: [string, string, string][] = [
    ...
    ["50 DMA", stock.dma50 != null ? inr(stock.dma50, { decimals: 0 }) : "—", stock.dma50 == null ? "—" : stock.price >= stock.dma50 ? "Above" : "Below"],
    ["200 DMA", stock.dma200 != null ? inr(stock.dma200, { decimals: 0 }) : "—", stock.dma200 == null ? "—" : stock.price >= stock.dma200 ? "Above" : "Below"],
...
function Peers({ peers, self }: { peers: Stock[]; self: Stock }) {
  ...
  const columns: Column<Stock>[] = [
    ...
    { key: "chg", header: "Chg%", align: "right", sortable: true, sortValue: (s) => s.changePct, cell: (s) => <span className={cn("tnum font-medium", trendClass(s.changePct))}>{pct(s.changePct)}</span> },
